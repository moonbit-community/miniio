---
name: miniwasi-quick-eval
description: Prototype small MoonBit WASIp1 stdin/stdout filters with moon run -c and moonbit-community/miniwasi, especially jq-style JSON transforms, without creating a package.
---

# Miniwasi Quick Eval

Use this when the user wants a quick one-off MoonBit WASIp1 script with
`moon run -c`, especially for stdin/stdout filters, JSON inspection, or
small miniwasi API experiments that do not deserve a full package.

## Pattern

- Use MoonBit's single-file import header at the top of the `-c` source.
- Import `moonbit-community/miniwasi/io` when using reader, writer, or data
  extension methods such as `read_all()`, `write()`, and `.text()`.
- Prefer `stdin.read_all().text()` for UTF-8 stdin text.
- Write normal output to `@wasi.stdout`; write diagnostics to `@wasi.stderr`
  and exit nonzero with `@wasi.exit(1)`.
- Keep these scripts small. If the source grows beyond one screen or needs
  files/assets/tests, create a normal package instead.

## JSON Filter Example

```sh
printf '{"name":"miniwasi","ok":true}' | moon run -c 'import {
  "moonbit-community/miniwasi" @wasi,
  "moonbit-community/miniwasi/io",
  "moonbitlang/core/json" @json,
}

fn main {
  try {
    let input = @wasi.stdin.read_all().text()
    let value = @json.parse(input)
    @wasi.stdout.write_text(value.stringify(indent=2) + "\n")
  } catch {
    e => {
      @wasi.stderr.write_text("jq failed: \{e}\n") catch { _ => () }
      @wasi.exit(1)
    }
  }
}'
```

Expected output:

```json
{
  "name": "miniwasi",
  "ok": true
}
```

## Validation

For syntax-only examples, test the same shape with a registry-resolvable package
or a temporary local package. For released miniwasi usage, the command itself is
the validation.
